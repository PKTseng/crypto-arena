    ...
    <div className="flex flex-col items-center text-center gap-16 py-12">
      ...
      <section className="flex flex-col items-center gap-6 max-w-2xl">
        ...
        <h1 className="text-5xl sm:text-6xl font-black tracking-tight text-glow-yellow">
          Crypto<span className="text-yellow-400">Arena</span>
        </h1>
        ...
        <Link
          href="/game"
          className="inline-flex items-center gap-2 bg-yellow-500 hover:bg-yellow-400 text-black font-bold px-8 py-4 rounded-2xl text-lg transition-colors duration-150 shadow-[0_0_30px_rgba(234,179,8,0.3)]"
        >
          開始遊戲 <ArrowRight size={20} />
        </Link>
      </section>
      ...
      <section className="grid sm:grid-cols-3 gap-6 w-full max-w-4xl">
        {features.map((f) => (
          <div
            key={f.title}
            className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 text-left hover:border-gray-700 transition-colors"
          >
      ...
      <section className="max-w-xl w-full">
        <h2 className="text-2xl font-bold mb-6 text-gray-200">遊戲規則</h2>
        <ol className="flex flex-col gap-4 text-left">
          {[
            ...
            <li key={i} className="flex gap-4 items-start">
              <span className="w-7 h-7 rounded-full bg-yellow-500/20 text-yellow-400 font-bold text-sm flex items-center justify-center shrink-0 mt-0.5">
                {i + 1}
              </span>
